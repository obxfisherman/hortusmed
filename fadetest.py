...
def setfade(asset_list, id, delta, offset):
...
class GrfxAsset(pygame.sprite.Sprite):
    ...
    def update(self):       
        if self.offset[X]<0: self.offset=(self.offset[X] + 1, self.offset[Y])
        if self.offset[X]>0: self.offset=(self.offset[X] - 1, self.offset[Y])
        if self.offset[Y]<0: self.offset=(self.offset[X], self.offset[Y] + 1)
        if self.offset[Y]>0: self.offset=(self.offset[X], self.offset[Y] - 1)
